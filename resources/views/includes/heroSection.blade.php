
        <!-- Hero Section Start -->
        <section class="hero-section-2 hero-2 hero-3">
            <div class="array-button">
                <button class="array-prev">
                    <i class="far fa-chevron-left"></i>
                </button>
                <button class="array-next">
                    <i class="far fa-chevron-right"></i>
                </button>
            </div>
            <div class="swiper banner-active">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <div class="hero-height">
                            <div class="hero-bg bg-cover" style="background-image: url('img/home-3/hero/hero-bg.jpg');"></div>
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="hero-content">
                                        <h1>
                                            <span class="shape-1">
                                                <img src="img/home-3/hero/shape-1.png" alt="img">
                                            </span>
                                            <span class="text">Powerful</span> <br> Solution Protect <br> Environment
                                        </h1>
                                    </div>
                                    <div class="hero-button">
                                        <a href="service.html" class="theme-btn">
                                            View Our Services
                                            <i class="fa-solid fa-arrow-up-right"></i>
                                        </a>
                                    </div>
                                    <div class="hero-feature-area">
                                        <div class="hero-feature-items">
                                            <div class="icon-text">
                                                <div class="icon">
                                                    <img src="img/home-3/hero/icon1.svg" alt="img">
                                                </div>
                                                <h3>Expert Our <br> Installation Team</h3>
                                            </div>
                                            <div class="feature-number">
                                                <h6>01</h6>
                                                <a href="about.html" class="arrow-icon">
                                                    <i class="fal fa-long-arrow-up"></i>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="hero-feature-items">
                                            <div class="icon-text">
                                                <div class="icon">
                                                    <img src="img/home-3/hero/icon2.svg" alt="img">
                                                </div>
                                                <h3>24 Hours Our <br> Clients Supports</h3>
                                            </div>
                                            <div class="feature-number">
                                                <h6>02</h6>
                                                <a href="about.html" class="arrow-icon">
                                                    <i class="fal fa-long-arrow-up"></i>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="hero-height">
                            <div class="hero-bg bg-cover" style="background-image: url('img/home-3/hero/hero-bg-2.jpg');"></div>
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="hero-content">
                                        <h1>
                                            <span class="shape-1">
                                                <img src="img/home-3/hero/shape-1.png" alt="img">
                                            </span>
                                            <span class="text">Powerful</span> <br> Solution Protect <br> Environment
                                        </h1>
                                    </div>
                                    <div class="hero-button">
                                        <a href="service.html" class="theme-btn">
                                            View Our Services
                                            <i class="fa-solid fa-arrow-up-right"></i>
                                        </a>
                                    </div>
                                    <div class="hero-feature-area">
                                        <div class="hero-feature-items">
                                            <div class="icon-text">
                                                <div class="icon">
                                                    <img src="img/home-3/hero/icon1.svg" alt="img">
                                                </div>
                                                <h3>Expert Our <br> Installation Team</h3>
                                            </div>
                                            <div class="feature-number">
                                                <h6>01</h6>
                                                <a href="about.html" class="arrow-icon">
                                                    <i class="fal fa-long-arrow-up"></i>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="hero-feature-items">
                                            <div class="icon-text">
                                                <div class="icon">
                                                    <img src="img/home-3/hero/icon2.svg" alt="img">
                                                </div>
                                                <h3>24 Hours Our <br> Clients Supports</h3>
                                            </div>
                                            <div class="feature-number">
                                                <h6>02</h6>
                                                <a href="about.html" class="arrow-icon">
                                                    <i class="fal fa-long-arrow-up"></i>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>